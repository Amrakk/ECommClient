...
import { Box, Tabs, Tab, Fade, Typography, Table, TableBody, TableRow, TableCell, Chip, Card } from "@mui/material";
import { useEffect, useState } from "react";
import Grid from "@mui/material/Grid2";
import ReviewCard from "./Product/ReviewGeneralCompomemt";
                ...
                {value === 2 && (
                    <Grid container spacing={2} sx={{ my: 10 }} >
                        <Grid size={{ xs: 12, md: 5 }}>
                            {isLoadingFetchRating ? <ReviewCard /> : productRatingsResult.length === 0 ? <Typography variant="h6"></Typography> : (
                                <ReviewCard totalResult={productRatingsResult} />
                            )}
                        </Grid>
                        <Grid size={{ xs: 12, md: 7 }} sx={{ display: 'flex', flexDirection: 'column', gap: 3 }} >
                            {isLoadingFetchRating ? <UserReviewCard />
                                : productRatingsResult.length === 0 ? <Typography variant="body1">Be the first to review this product</Typography>
                                    : (
