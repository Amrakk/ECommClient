        ...
        >
            <CardContent>
                <Stack direction="row" spacing={2} alignItems="center" mb={2}>
                    {isLoading ? (
                        <Skeleton variant="circular" animation="wave" sx={{ width: 40, height: 40 }} />
                    ) : (
                        <Avatar sx={{ bgcolor: deepOrange[500] }} src={review?.user.avatarUrl ?? DEFAULT_AVATAR_URL} />
                    )}
                    {isLoading ? (
                ...
                {isLoading ? (
                    <Skeleton variant="rounded" animation="wave" sx={{ height: 70 }} />
                ) : (
                    <>
