import { Box, Typography, Card, CardContent, Divider, Button, Grow, Skeleton } from "@mui/material";
...
const TransactionDetailPage = () => {
    ...
    const { data: transactionOrder, isError: isErrorTransactionOrder } = useGetTransactionByOrderIdQuery(
        urlParams.orderCode || ""
    );
                ...
                <Box sx={{ m: 5, mt: 10 }}>
                    <Grid container spacing={2} sx={{ display: "flex", justifyContent: "center", gap: 4 }}>
                        <Skeleton
                            ...
                            variant="rectangular"
                            height={100}
                            sx={{ borderRadius: 3, width: { xs: "100%", lg: "50%" } }}
                        />
                        <Skeleton
                            animation="wave"
                            variant="rectangular"
                        ...
                        <Typography variant="body1" sx={{ color: "#b3b3b3", marginTop: 1 }}>
                            {isUsingParams ? (
                                urlParams.cancel === true ? (
                                    ...
                                    <>
                                        The order invoice has been sent to <strong>{user!.email}</strong>
                                    </>
                                )
                            ) : (
                                <>
                        ...
                        <CardContent sx={{ padding: { xs: 2, md: 4 } }}>
                                ...
                                {["Transaction Date", "Payment Method", "Shipping Method"].map((label, index) => (
                                    <Grid size={{ xs: 12, md: 4 }} key={label}>
                                        ...
                                        >
                                            <Typography
                                                variant="caption"
                                                sx={{ color: "#878787", textTransform: "uppercase", letterSpacing: 1 }}
                                            >
                                                {label}
                                            </Typography>
                                            <Typography
                                                variant="body1"
                                                sx={{ fontWeight: 500, marginTop: 1, color: "#ffffff" }}
                                            >
                                                {index === 0 &&
                                                    `${formatDate(new Date(resCheckoutCopy!.transaction.createdAt))}`}
                                                {index === 1 &&
                                                    `${resCheckoutCopy!.transaction.paymentType} (${
                                                        resCheckoutCopy!.transaction.paymentType === PAYMENT_TYPE.COD
                                                            ? "Cash on delivery"
                                                            : "Online payment"
                                                    })`}
                            ...
                            <Box sx={{ marginTop: 4, backgroundColor: "#252525", padding: 3, borderRadius: 2 }}>
                                {[
                                    {
                                        label: "Subtotal",
                                        value: `${convertToVietnameseDong(resCheckoutCopy!.order.totalPrice)}`,
                                    },
                                    {
                                        label: "Discount",
                                        value: `${convertToVietnameseDong(
                                            resCheckoutCopy!.order.voucherDiscount || 0
                                        )}`,
                                        color: "#55efc4",
                                    },
                                    {
                                        label: "Loyal Point Discount",
                                        value: `${convertToVietnameseDong(
                                            (resCheckoutCopy!.order.loyaltyPointsDiscount || 0) * 1000
                                        )}`,
                                        color: "#55efc4",
                                    },
                                    {
                                        label: "Shipment cost",
                                        value: `${convertToVietnameseDong(resCheckoutCopy!.transaction.shippingFee)}`,
                                        color: "#ff7675",
                                    },
                                ].map((item) => (
                                    <Box
                                ...
                                >
                                    ...
                                    <Typography variant="h5" sx={{ fontWeight: 700, color: "#81ecec" }}>
                                        {convertToVietnameseDong(
                                            resCheckoutCopy!.transaction.paymentAmount +
                                                resCheckoutCopy!.transaction.shippingFee -
                                                (resCheckoutCopy!.order.voucherDiscount || 0) -
                                                (resCheckoutCopy!.order.loyaltyPointsDiscount || 0) * 1000
                                        )}
                                    </Typography>
