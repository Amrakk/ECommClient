        ...
        <Grid container spacing={2} sx={{ m: 5 }}>
            ...
            <Grid container spacing={2} size={3} sx={{ display: { md: 'block', xs: 'none' } }}  >
                ...
                <Grid size={12} sx={{
                    ...
                    <FiltersComponent  setProductsData={setDataProduct} category={props.category} productsData={dataProduct} />
                </Grid>
            </Grid>
            {/* Second Column */}
            <Grid container spacing={2.5} size={{ lg: 9, md: 9, sm: 12, }}
                sx={{ display: 'flex', pl: 2 }} >
                <Grid size={12} sx={{ height: '5%' }} > <Typography> Showing {totalProductPerPage} results in total {dataProduct.length} results  </Typography> </Grid>
                {isLoading ? <>
                ...
                {
                    dataProduct.length > totalProductPerPage && <Grid size={12} sx={{ display: 'flex', justifyContent: 'center', mt: 2 }}>
                        <Pagination count={Math.ceil(dataProduct.length / totalProductPerPage)} page={page} onChange={(e,value) => handleChangePage(value)} />
                    </Grid>
                }
            </Grid>
        </Grid>
